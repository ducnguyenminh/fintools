...
def format_percentage(rate, decimal_places=4):
    """利率を%表記でフォーマット"""
...
def example_1_rate_conversion():
    ...
    print(f"年利 {format_percentage(annual_rate, 2)} → 月利 {format_percentage(monthly_rate, 4)}")
    
    # 確認のため月利から年利に戻す
    back_to_annual = monthly_to_annual_rate(monthly_rate)
    print(f"月利 {format_percentage(monthly_rate, 4)} → 年利 {format_percentage(back_to_annual, 2)}")
    
    print(f"\n注意: 単純に12で割った場合の月利は {format_percentage(annual_rate/12, 4)} となり、")
    print("複利効果を考慮した実効月利とは異なります。")
    print()


def example_2_compound_interest():
    """例2: 複利計算比較"""
    print("=" * 50)
    ...
    for freq, description in frequencies:
        ...
        print(f"{description:20}: {format_currency(result)} (利息: {format_currency(result - principal)})")
    
    # 連続複利
    continuous_result = continuous_compounding(principal, annual_rate, years)
    print(f"{'連続複利':20}: {format_currency(continuous_result)} (利息: {format_currency(continuous_result - principal)})")
    
    print(f"\n複利回数が多いほど将来価値が大きくなることがわかります。")
    print()


def example_3_investment_scenarios():
    """例3: 投資シナリオ比較"""
    print("=" * 50)
    print("例3: 投資シナリオ比較")
    print("=" * 50)
    ...
    for rate, description in scenarios:
        ...
        print(f"{description}")
        print(f"  年次複利: {format_currency(result_annual)} (利息: {format_currency(result_annual - principal)})")
        print(f"  月次複利: {format_currency(result_monthly)} (利息: {format_currency(result_monthly - principal)})")
        print(f"  実効年利: {format_percentage(effective_annual_rate(rate, 12), 2)}")
        print()


def example_4_present_value():
    """例4: 現在価値計算"""
    print("=" * 50)
    print("例4: 現在価値計算")
    print("=" * 50)
    ...
    print(f"確認: {format_currency(required_investment)} を年利{format_percentage(annual_rate, 1)}で{years}年運用すると")
    print(f"      {format_currency(check_amount)} になります。")
    print()


def example_5_monthly_investment():
    """例5: 毎月積立投資シミュレーション"""
    print("=" * 50)
    ...
    total_amount = monthly_investment * (((1 + monthly_rate) ** total_months - 1) / monthly_rate)
