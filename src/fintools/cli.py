...
def cmd_compound_interest(args: argparse.Namespace) -> None:
    ...
    try:
        result = compound_interest(
            args.principal, args.annual_rate / 100, args.years, args.frequency
        )
...
def cmd_present_value(args: argparse.Namespace) -> None:
    ...
    try:
        result = present_value(
            args.future_value, args.annual_rate / 100, args.years, args.frequency
        )
...
使用例:
  ...
  fintools compound 1000000 5.0 10 -f 12  # 月次複利で計算
    ...
    # 年利→月利変換
    parser_a2m = subparsers.add_parser("annual-to-monthly", help="年利から月利に変換")
    parser_a2m.add_argument("annual_rate", type=float, help="年利（%表記、例：5.0）")
    parser_a2m.set_defaults(func=cmd_annual_to_monthly)

    # 月利→年利変換
    parser_m2a = subparsers.add_parser("monthly-to-annual", help="月利から年利に変換")
    parser_m2a.add_argument("monthly_rate", type=float, help="月利（%表記、例：0.4）")
    parser_m2a.set_defaults(func=cmd_monthly_to_annual)

    # 複利計算
    parser_compound = subparsers.add_parser("compound", help="複利計算")
    parser_compound.add_argument("principal", type=float, help="元本（円）")
    parser_compound.add_argument(
        "annual_rate", type=float, help="年利（%表記、例：5.0）"
    )
    parser_compound.add_argument("years", type=float, help="期間（年）")
    parser_compound.add_argument(
        "-f", "--frequency", type=int, default=1, help="年間複利回数（デフォルト：1）"
    )
    parser_compound.set_defaults(func=cmd_compound_interest)

    # 現在価値計算
    parser_pv = subparsers.add_parser("present-value", help="現在価値計算")
    parser_pv.add_argument("future_value", type=float, help="将来価値（円）")
    parser_pv.add_argument("annual_rate", type=float, help="年利（%表記、例：5.0）")
    parser_pv.add_argument("years", type=float, help="期間（年）")
